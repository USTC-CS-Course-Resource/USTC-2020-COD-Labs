...
.data
    .word 0,6,0,8,0x80000000,0x80000100,0x100,5,0   #编译成机器码时，编译器会在前面多加个0，所以后面lw指令地址会多�?4
                   ;
_start:    
...
_next2:
		add $0, $0, $t2			#$0应该�?直为0			56 	000a0020
		beq $0,$t3,_success		#						60	100b0002
		
		
_fail:  
		sw   $t3,8($0) #失败通过看存储器地址0x08里�?�，若为0则测试不通过，最初地�?0x08里�?�为0	ac0b0008
        j    _fail		# 08000010

_success: 
		sw   $t2,8($0)    #全部测试通过，存储器地址0x08里�?�为1	ac0a0008
		j   _success       		#08000012

					  #判断测试通过的条件是�?后存储器地址0x08里�?�为1，说明全部�?�过测试
